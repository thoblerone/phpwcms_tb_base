<?php
/*****************************************
 notiz: dies ist die bearbeitete Variante vom TSF aus Zeiten von phpwcms 1.6.3
        ggf. liegt eine aktuelle Version im disabled-Verzeichnis! 
*****************************************/
...
if($fe_action) {
	...
	if(isset($_POST['user_login'])) {
		...
		if($fe_action == '{FE_USER_REGISTER}') {
			...
			if( empty($udata['user_password']) ) {
				...
				$uerror['user_password'] = '<br>Passwort und Wiederholung sind nicht identisch';
			}
		...
		if( empty($udata['user_name']) ) {
			$uerror['user_name'] = 'Anzeigename muss ausgef&uuml;llt werden';
		}
		...
		if( empty($udata['user_company']) || strlen($udata['user_company']) < 5 ) {
			$uerror['user_company'] = 'Gruppe/Abteilung muss ausgef&uuml;llt werden';
		}
	...
	if($fe_action == '{FE_USER_REGISTER}') {
		...
		$fe_reg[] = '<p>Bitte w&auml;hlen Sie Ihr Login aus.</p>';
		
	} else {
	
		$fe_reg[] = '<p>Hier k&ouml;nnen Sie Ihre Profildaten bearbeiten.</p>';
	...
	$fe_reg[] = '<fieldset>';
	$fe_reg[] = '<legend> Login Daten </legend>';
	
	$fe_reg[] = "<table>";
	$fe_reg[] = "<tr>";
	$fe_reg[] = "<td width='170px'>";
	...
	$fe_reg[] = '<tr><td width="170px">';
	$fe_reg[] = '<label class="labelpos" for="user_password">Passwort: </label></td>';
	$fe_reg[] = '<td>';
	...
	$fe_reg[] = '<tr>';
	$fe_reg[] = '<td width="170px"><label class="labelpos" for="user_password2">Passwort erneut eingeben:</label></td>';
	$fe_reg[] = '<td><input type="password" name="user_password2" id="user_password2" class="textfield" maxlength="20" size="30" />';
	$fe_reg[] = '</td></tr></table>';
	$fe_reg[] = '</fieldset>';
	
	$fe_reg[] = '<fieldset>';
	$fe_reg[] = '<legend> Anzeige-Informationen </legend>';
	$fe_reg[] = "<table>";
	// $fe_reg[] = '<p>';
	...
	$fe_reg[] = "<tr>";
	$fe_reg[] = "<td width='170px'>";
	$fe_reg[] = '<label class="labelpos" for="user_name">Anzeigename: </label>';
	$fe_reg[] = "</td>";
	$fe_reg[] = "<td>";
	...
	$fe_reg[] = "<tr>";
	$fe_reg[] = "<td width='170px'>";	
	$fe_reg[] = '<label class="labelpos" for="user_company">Gruppe/Abteilung:</label>';
	$fe_reg[] = "</td>";
	$fe_reg[] = "<td>";
	...
	$fe_reg[] = "<tr>";
	$fe_reg[] = "<td width='170px'>";
	$fe_reg[] = '<label class="labelpos" for="user_tel">Telefon: </label>';
	$fe_reg[] = "</td>";
	$fe_reg[] = "<td>";
//	$fe_reg[] = is_uerror('user_tel');
	...
	$fe_reg[] = "<tr>";
	$fe_reg[] = "<td>";
	$fe_reg[] = '<label class="labelpos" for="user_email">E-Mail: </label>';
	$fe_reg[] = "</td>";
	$fe_reg[] = "<td>";
	...
	// if($fe_action == '{FE_USER_REGISTER}') {
		...
		// $fe_reg[] = 'Wie l&auml;sst sich Ihr Gesch&auml;ftsbereich am besten beschreiben?';
		// $fe_reg[] = '</p>';
		...
		// $fe_reg[] = '<p class="title">';
		// $fe_reg[] = 'Wieviele Mitarbeiter besch&auml;ftigt Ihr Unternehmen?';
		// $fe_reg[] = '</p>';
		...
		// $fe_reg[] = '<p class="title">';
		// $fe_reg[] = 'In welchen Branchen sind Ihre Kunden haupts&auml;chlich t&auml;tig?';
		// $fe_reg[] = '</p>';
	
		// $fe_reg[] = '<p>';
		// foreach(array(	'Automotive', 'Banken & Versicherungen', 'Energie', 
						// 'ITK', 'Großhandel', 'Einzelhandel', 'Fertigung', 
						// 'Gesundheit', 'Medien', '&ouml;ffentliche Einrichtungen', 
	...
	$fe_reg[] = '<p>';
	$fe_reg[] = '<input type="submit" value="Senden" class="button" /><input type="hidden" name="RemoteIP" value="'.$_SERVER['REMOTE_ADDR'].'" />';
	$fe_reg[] = '</p>';
	...
	if(isset($_POST['user_login']) && $fe_action == '{FE_USER_REGISTER}') {
		if($uerror['status']) {
			...
			$sql .= 'detail_title, detail_lastname,  ';
			$sql .= 'detail_fon, detail_aktiv, detail_varchar1, detail_email, detail_login, detail_password,  detail_company) VALUES (';
			...
			if(!empty($queryResult['INSERT_ID'])) {
				...
				$fe_text .= LF;
				$fe_text .= 'Folgende Zugangsdaten sind von Ihnen gesendet worden:' . LF;
				$fe_text .= LF;

				$fe_text .= '  Login:    ' . $udata['user_login'] . LF;
				$fe_text .= '  Passwort: ' . $udata['user_password'] . LF . LF;
				$fe_text .= LF;
				$fe_text .= 'Ihr Passwort ist nicht reproduzierbar verschlüsselt in unserem System abgelegt worden.' . LF . LF . LF;
				$fe_text .= LF;
				...
				$fe_text1 .= 'Die Benutzerdaten können nach dem Login im Backend eingesehen werden.' . LF;
				$fe_text1 .= 'http://www.tanzsportfreunde-essen.de/phpwcms.php?do=modules&module=personel' . LF;
				...
				$fe_text1 .= 'Abteilung: ' . $udata['user_company'] . LF;
				...
				$fe_text1 .= LF;
				$fe_text1 .= LF . '-----------------------------------------------------------' . LF;
				$fe_text1 .= 'IP: '. $udata['RemoteIP']. '    ' .getRemoteIP();
				$fe_text1 .= LF . '-----------------------------------------------------------' . LF;
	...
	if(isset($_POST['user_login']) && $fe_action == '{FE_USER_MANAGE}') {
		...
		if($uerror['status']) {
			...
			if(isset($queryResult['AFFECTED_ROWS'])) {
				...
				$fe_reg = '<p>Ihre Profildaten wurden erfolgreich aktualisiert.</p>' . LF . 
				          '<p>Damit eine Namens&auml;nderung wirksam wird, loggen Sie sich bitte aus und wieder ein.</p>' . LF . $fe_reg;
...
function is_uerror($field='') {
	...
	if(!empty($uerror[$field])) {
		$uerror['status'] = true;
		return '<span style="color: red">' . $uerror[$field] . '</span>';
	}
	return '';
}
